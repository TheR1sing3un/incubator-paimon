...
def _extract_or_clause(or_pred: Predicate,
    ...
    """For one AND-child predicate, return either:
      ...
      * None                            — clause is not a bucket-key
        constraint we can safely use; caller skips it.
...
def create_bucket_selector(
    ...
    for and_child in _split_and(predicate):
        ...
        if slot_values[slot] is not None:
            ...
            # so do we. (e.g. `id = 1 AND id = 2` is unsatisfiable but we
            # don't reason about that — any superset, including "all
            # buckets", is correct.)
            return None
