...
class FileScanner:
    ...
    def _init_bucket_selector(self):
        ...
        ``FixedBucketRowKeyExtractor`` and reading back its resolved fields.
        Reusing the writer class (rather than re-deriving the list here) is
        what guarantees the reader always hashes against the same fields
        the writer used at insert time — any future change to the writer's
        bucket-key resolution propagates automatically. Mirrors the
        precedent in ``pypaimon/ray/shuffle.py`` (see Phase 1 review).
        ...
        if self.predicate is None:
            return None
        # bucket_mode() returns HASH_FIXED when options.bucket() > 0 (PK or
        # AO); other modes (DYNAMIC / POSTPONE / UNAWARE / CROSS_PARTITION)
        ...
        try:
            ...
            # falls back to no pruning instead of crashing the scan.
            return None
        # Reuse the writer's bucket-key derivation directly. If construction
        # fails for any reason we fall back to no pruning rather than
        # diverging.
