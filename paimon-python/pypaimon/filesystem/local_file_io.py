...
class LocalFileIO(FileIO):
    ...
    def rename(self, src: str, dst: str) -> bool:
        ...
        # HadoopFileIO compatibility. Directory check under the in-process
        # lock is a hint; the file-level race is handled below by os.link's
        ...
        # already exists. Path.rename silently overwrites on POSIX, which
        # breaks cross-process create-if-absent semantics that commit
        # protocols rely on.
        try:
            ...
            # cost of cross-process atomicity (only RENAME_LOCK below
            # protects within a single process).
