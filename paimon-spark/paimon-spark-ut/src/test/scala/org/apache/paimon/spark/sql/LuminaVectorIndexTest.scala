...
class LuminaVectorIndexTest extends PaimonSparkTestBase {
  ...
  private lazy val isLuminaAvailable: Boolean = {
    try {
      ...
      println("[LuminaVectorIndexTest] Lumina native library loaded successfully")
      true
    } catch {
      case e: UnsatisfiedLinkError =>
        println(s"[LuminaVectorIndexTest] Lumina native library NOT available: ${e.getMessage}")
        println(
          s"[LuminaVectorIndexTest] os.name=${System.getProperty("os.name")}, " +
            s"os.arch=${System.getProperty("os.arch")}, " +
            s"java.library.path=${System.getProperty("java.library.path")}")
        false
      case e: Throwable =>
        println(
          s"[LuminaVectorIndexTest] Lumina class load failed: ${e.getClass.getName}: ${e.getMessage}")
        if (e.getCause != null) {
          println(
            s"[LuminaVectorIndexTest] Caused by: ${e.getCause.getClass.getName}: ${e.getCause.getMessage}")
        }
        false
    }
  }
